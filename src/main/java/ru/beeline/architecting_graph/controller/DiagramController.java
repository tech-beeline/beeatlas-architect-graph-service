...
public class DiagramController {
    ...
    public ResponseEntity<String> getDeploymentDiagramm(@PathVariable String environment,
        ...
        return ResponseEntity.ok().contentType(MediaType.APPLICATION_JSON).body(diagramService.createDiagram(softwareSystemMnemonic, null, environment, rankDirection));
    }
    ...
    public String getContextDiagramV2(@RequestParam String cmdb,
                                      @RequestParam(required = false) String rankDirection,
    ...
    public ResponseEntity<String> getC4Diagram(@PathVariable String softwareSystemMnemonic,
        ...
        return ResponseEntity.ok().contentType(MediaType.APPLICATION_JSON).body(diagramService.createDiagram(softwareSystemMnemonic, containerMnemonic, null, rankDirection));
    }

    @GetMapping("/diagram/deployment")
    @Operation(summary = "Построение Deployment диаграммы")
    public String getDiagramDeployment(@RequestParam String cmdb,
    ...
    @Operation(
            summary = "Построение elements",
            description = "получение внешних систем, которые вызывают систему (продукт) из запроса (системы которые зависят от системы из запроса)",
            responses = {
                    @ApiResponse(responseCode = "200", description = "Успешный ответ",
                            content = @Content(mediaType = "application/json",
                                    schema = @Schema(implementation = DiagramElementDTO.class)))
    ...
    @Operation(
            ...
            description = "это получение внешних систем, которые вызывает система (продукт) из запроса (системы от которых зависит система из запроса)",
            responses = {
                    @ApiResponse(responseCode = "200", description = "Успешный ответ",
                            content = @Content(mediaType = "application/json",
                                    schema = @Schema(implementation = String.class))),
