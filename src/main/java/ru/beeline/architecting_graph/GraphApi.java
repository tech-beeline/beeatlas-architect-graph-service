...
import ru.beeline.architecting_graph.graph.graphFunctions;
...
import ru.beeline.architecting_graph.otherObjects.Pair;
...
public class GraphApi {
    ...
    public ResponseEntity<String> LocalGraph(@PathVariable("docId") Long id_file) {
        ...
        try {
            // Построение графа
            Session session = driver.session();
            graphFunctions.createGraph(session, "Local", workspace);
        } catch (Exception e) {
            // Обработка исключения
    ...
    public ResponseEntity<String> GlobalGraph(@PathVariable("docId") Long id_file) {
        ...
        try {
            // Построение графа
            Session session = driver.session();
            graphFunctions.createGraph(session, "Global", workspace);
        } catch (Exception e) {
            // Обработка исключения
            return ResponseEntity.badRequest().body("Граф не построен" + '\n' + e);
        }
    ...
    public ResponseEntity<String> compareVersions(@PathVariable String cmdb_code, @PathVariable Integer v1,
        ...
        GraphObject systemGraphObject = GraphObject.createGraphObject("SoftwareSystem", "cmdb", cmdb_code);
