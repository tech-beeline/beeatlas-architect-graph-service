...
public class ContainerInstanceService {
    ...
    public void setParametersForContainerInstance(String graphTag,
                                                  ContainerInstance containerInstance,
                                                  GraphObject containerInstanceGraphObject,
                                                  String curVersion) {
        if (graphTag.equals("Global") && genericRepository.getObjectParameter(graphTag,
                                                                              containerInstanceGraphObject,
                                                                              "startVersion")
                .toString()
                .equals("NULL")) {
            genericRepository.setObjectParameter(graphTag, containerInstanceGraphObject, "startVersion", curVersion);
        }
        containerInstanceRepository.updateContainerInstance(graphTag,
    ...
    public void updateContainerInstance(String graphTag,
                                        Model model,
        ...
        GraphObject containerInstanceGraphObject = new GraphObject("ContainerInstance", "name", containerInstanceName);
    ...
    public void setContainerInstanceEndVersion(String graphTag, String deploymentNodeName, String curVersion) {
        Result result = containerInstanceRepository.findContainerInstanceNamesWithNullEndVersion(graphTag,
                                                                                                 deploymentNodeName);
    ...
    public void updateContainerInstanceRelationships(String graphTag,
                                                     DeploymentNode deploymentNode,
                                                     String curVersion,
                                                     String cmdb,
                                                     Model model,
            ...
            for (ContainerInstance containerInstance : deploymentNode.getContainerInstances()) {
                if (containerInstance.getRelationships() != null) {
                    for (RelationshipEntity relationship : containerInstance.getRelationships()) {
                        createExternalObjects.updateDefaultRelationship(graphTag,
                                                                        relationship,
                                                                        model,
                                                                        curVersion,
                                                                        cmdb,
    ...
    public List<TcDTO> getContainerInstancesByDeploymentNodeId(Integer id) {
        ...
        return productClient.getTechCapabilitiesByContainerProduct(aliasProduct, originalNames);
    }
}
