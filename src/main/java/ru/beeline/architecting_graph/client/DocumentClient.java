...
public class DocumentClient {
    ...
    public String getDocument(Long docId) {
        try {
            ...
            throw new ServiceUnavailableException("Ошибка при вызове документа: " + e.getStatusCode());
        } catch (HttpClientErrorException.Forbidden e) {
            log.error("Документ недоступен (403 Forbidden): ", e);
            throw new DocumentForbiddenException("Нет прав на получение документа");
        } catch (HttpClientErrorException.NotFound e) {
            log.error("Запись с данным id не найдена: ", e);
            throw new NotFoundException("Файла с указанным id не существует");
        } catch (RestClientException e) {
            throw new ServiceUnavailableException("Ошибка при вызове документа");
        }
    }
}
